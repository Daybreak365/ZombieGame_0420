...
import sinsa.zombie.utils.collect.QueueOnIterateHashSet;
...
public class SimpleTimer {
	...
	public boolean start() {
		if (!isRunning()) {
			...
			this.bukkitTask = Bukkit.getScheduler().runTaskTimer(ZombieGame.getInstance(), task, initialDelay, period);
			if (observers != null) {
	...
	public boolean resume() {
		if (isPaused()) {
			this.bukkitTask = Bukkit.getScheduler().runTaskTimer(ZombieGame.getInstance(), task, 0, period);
			if (observers != null) {
				for (final Observer observer : observers) {
					observer.onResume();
				}
